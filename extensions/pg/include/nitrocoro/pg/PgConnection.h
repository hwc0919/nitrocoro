...
public:
    ...
    // Prefer statement_timeout (PgConnectConfig::statementTimeoutMs) for query timeouts —
    // PostgreSQL will abort the query server-side and the connection remains usable.
    // cancelToken only stops waiting for the result; the query continues running on the server
